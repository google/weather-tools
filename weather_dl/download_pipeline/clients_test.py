...
class MaxWorkersTest(unittest.TestCase):
    def test_cdsclient_internal(self):
        ...
        self.assertEqual(
            client.num_requests_per_key("reanalysis-era5-some-data", False), 5)

    def test_cdsclient_mars_hosted(self):
        client = CdsClient(Config.from_dict({'parameters': {'api_url': 'url', 'api_key': 'key'}}))
        self.assertEqual(
            client.num_requests_per_key("reanalysis-carra-height-levels", False), 2)

    def test_marsclient(self):
        client = MarsClient(Config.from_dict({'parameters': {}}))
        self.assertEqual(
            client.num_requests_per_key("reanalysis-era5-some-data", False), 2)

    def test_fakeclient(self):
        client = FakeClient(Config.from_dict({'parameters': {}}))
        self.assertEqual(
            client.num_requests_per_key("reanalysis-era5-some-data", False), 1)
