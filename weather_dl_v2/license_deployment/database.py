...
class FirestoreClient(Database, CRUDOperations):
    ...
    def _get_config_from_queue_by_license_id(self, license_id: str) -> str | None:
        result: DocumentSnapshot = self._get_db().collection('queues').document(license_id).get(['queue'])
        if result.exists:
...
def get_partition_from_manifest(transaction, config_name: str) -> str | None:
    ...
    snapshot = db_client._get_db().collection('test_manifest').where(filter=and_filter)\
    ...
    ref: DocumentReference = db_client._get_db().collection(u'test_manifest').document(snapshot.id)
