...
class PartitionConfig():
    ...
    def loop_through_subsections(self, it: Config) -> Partition:
    ...
    def _create_partition_config(self, option: t.Tuple, config: Config) -> Config:
    ...
    def skip_partition(self, config: Config, store: Store) -> bool:
    ...
    def prepare_partitions(self) -> t.Iterator[Config]:
        ...
        for option in itertools.product(*[self.config.selection[key] for key in self.config.partition_keys]):
            yield self._create_partition_config(option, self.config)
    ...
    def new_downloads_only(self, candidate: Config) -> bool:
        ...
        if self.store is None:
            store = FSStore()
        should_skip = self.skip_partition(candidate, store)
        if should_skip:
            print("Skipped.")
    ...
    def assemble_config(self, partition: Partition) -> Config:
        ...
        print(f'[{name}] Created partition {location!r}.')
