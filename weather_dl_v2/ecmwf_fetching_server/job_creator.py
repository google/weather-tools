...
def create_download_job(message):
    ...
    config_name, selection, user_id, url, target_path = parsed_message.values()
    ...
    with open(path.join(path.dirname(__file__), "downloader.yaml")) as f:
        ...
        dep['spec']['template']['spec']['containers'][0]['command'] = ['python', 'downloader.py', config_name, selection, user_id, url, target_path]
