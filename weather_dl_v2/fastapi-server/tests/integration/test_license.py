...
app.dependency_overrides[get_terminate_license_deployment] = get_terminate_license_deployment_mock
...
def _add_license(headers, payload, code, expected):
    response  = client.post("/license", headers=headers, data=json.dumps(payload), params={"license_id": "L1"})
...
def _update_license(headers, license_id, license, code, expected):
    response = client.put(f"/license/{license_id}", headers=headers, data=json.dumps(license))
